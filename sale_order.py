...
class SaleOrder(models.Model):
    ...
    team_id = fields.Many2one(comodel_name='service.team', string='Team', ondelete='cascade')
    ...
    # @api.model
    def cek_overlap_time(self, *args, **kwargs):
        if self.team_id and self.booking_start and self.booking_end:
            ...
            if active_work_orders:
               raise UserError(f"Team already has a work order during that period on SO {self.name}")
            else:
