...
class WorkOrder(models.Model):
    ...
    def create(self, vals):
        ...
        # team_members_ids is a valid or not
        team_members_ids = []
        team_members_vals = vals.get('team_members_ids', [])
        if len(team_members_vals) > 0 and len(team_members_vals[0]) > 2:
            team_members_ids = [member_id for member_id in team_members_vals[0][2] if isinstance(member_id, int)]
